...
export function init(id, prevList) {
    ...
    EventHandler.on(viewer.closeButton, 'click', () => {
        ...
        viewer.resetImage()
        viewer.element.classList.remove('show')
    })
    ...
    EventHandler.on(viewer.fullScreen, 'click', () => {
        viewer.resetImage()
        viewer.element.classList.toggle('active')
    })
    ...
    EventHandler.on(document, 'keydown', e => {
        if (e.key === "ArrowUp") {
            ...
            const prevButton = viewer.element.querySelector('.bb-viewer-prev')
            if (prevButton) {
                prevButton.click()
            }
        } else if (e.key === "ArrowRight") {
            const nextButton = viewer.element.querySelector('.bb-viewer-next')
            if (nextButton) {
                nextButton.click()
            }
